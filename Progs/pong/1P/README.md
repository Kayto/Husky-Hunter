...
![PONGGAME running on a Husky Hunter](PONG7_SCREEN_PREVIEW.png)

## Status

**v0.1 — Working.** Based on PONG7 development iteration (7 iterations, collision + beep). Single player paddle.
...
| File | Description |
| ---- | ----------- |
| `gen_ponggame.py` | Python generator — assembles Z80 MC and outputs `PONGGAME.BAS` + `HBA/PONGGAME.HBA` |
| `PONGGAME.BAS` | BASIC listing — loads MC, draws net, runs game |
| `HBA/PONGGAME.HBA` | Tokenised version for transfer via HCOM |
| `PONG7_SCREEN_PREVIEW.png` | Screenshot shown at top of this README |
| `ponggame_frame.png` | Additional preview image |
| `PONGGAME_ASM/` | Z80 source, listing, decimal listing, sprite data — see [PONGGAME_ASM/ASM_README.md](PONGGAME_ASM/ASM_README.md) |

## Usage

On the Husky Hunter:

1. Transfer `HBA/PONGGAME.HBA` to the Hunter via HCOM over RS-232
2. Enter `BAS`
3. `LOAD "PONGGAME"`
4. `RUN`
...
Full development history (PONG through PONG7, 7 iterations) is documented in [Dev/pong/PONG\_DEV.md](../../Dev/pong/PONG_DEV.md).